...
function estado(d: DownloadEntry): string {
  if (d.state === 'progressing') {
    ...
    return d.paused ? 'En pausa' : d.total > 0 ? `${pct}% · ${fmtBytes(d.total)}` : fmtBytes(d.received)
  }
  if (d.state === 'completed') return fmtBytes(d.total || d.received)
  if (d.state === 'cancelled') return 'Cancelada'
  return 'Falló'
}
...
function DownloadsPopover(): JSX.Element {
  ...
  return (
    <PopoverPanel onHeight={dp.reportHeight} measure={list}>
      <PopoverLabel>
        {enCurso.length > 0 ? `Descargando ${enCurso.length}` : 'Descargas'}
      </PopoverLabel>

      {visibles.length === 0 ? (
        <div className="px-2.5 py-6 flex flex-col items-center gap-1.5 text-center">
          <IconDownload className="w-5 h-5 text-text-faint" />
          <span className="text-[12.5px] text-text-dim">Todavía no has descargado nada</span>
        </div>
      ) : (
        <PopoverList max={300}>
      ...
      {list.length > 0 && (
        <PopoverRow
          ...
          label="Limpiar la lista"
          title="Quita las descargas terminadas de la lista. No borra ningún archivo."
          onClick={dp.clear}
        />
      )}
    </PopoverPanel>

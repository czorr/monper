...
function Editor({ bm, onGuardar, onCancelar }: {
  ...
  return (
    <div className="px-4 py-3 flex flex-col gap-2 bg-white/[0.03]">
      ...
      <input value={url} onChange={(e) => setUrl(e.target.value)} onKeyDown={teclas} placeholder="https://…" className={campo} />
...
function BookmarksPage(): JSX.Element {
  ...
  const filtrados = useMemo(() => {
    ...
    if (!t) return items
    return items.filter((b) => b.title.toLowerCase().includes(t) || b.url.toLowerCase().includes(t))
  }, [items, q])
                ...
                editando === b.id ? (
                  ...
                  <div key={b.id} className="group/b flex items-center gap-3 px-4 py-2.5 hover:bg-white/[0.03] transition-colors">
                    ...
                    <button onClick={() => monperTab.navigate(b.url)} className="flex-1 min-w-0 text-left" title={b.url}>
                      ...
                      <span className="block truncate text-[12px] text-text-faint">{dominio(b.url)}</span>
                    </button>

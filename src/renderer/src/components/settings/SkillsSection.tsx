...
export default function SkillsSection(): JSX.Element {
  ...
  const select = (id: string | null): void => {
    ...
    if (dirty && !window.confirm(tr("¿Descartar los cambios sin guardar?"))) return
  ...
  const save = async (): Promise<void> => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : tr("No se pudo guardar la skill."))
  ...
  const openFolder = async (): Promise<void> => {
    ...
    catch (e) { setError(e instanceof Error ? e.message : tr("No se pudo abrir la carpeta.")) }
  }
  ...
  useEffect(() => {
    ...
    if (sel) void titanioTab.skillsGet(sel)
      .then((next) => {
        if (!live) return
        setDetail(next)
        if (!next) setError(tr("La skill ya no está disponible."))
      })
      .catch((error) => {
        console.error('[skills] no se pudo cargar el detalle:', error)
        if (live) setError(tr("No se pudo cargar la skill. Inténtalo de nuevo."))
      })
    return () => { live = false }
  }, [sel])
    ...
    <div className="h-full flex">
      ...
      <div className="w-[268px] shrink-0 border-r border-white/[0.06] flex flex-col">
        <div className="flex items-center justify-between px-3.5 h-12 shrink-0">
          <span className="text-[15px] font-semibold">{tr("Skills")}</span>
          <Button
            onClick={() => titanioTab.openSkillsFolder()}
            title={tr("Abrir la carpeta de skills en Finder")}
        ...
        <div className="px-2.5 pb-2">
          <Button className={actionRow}><IconFolderPlus /> {tr("Import from folder")}</Button>
          <Button className={actionRow}><IconPencil /> {tr("Create new skill")}</Button>
        </div>

        <div className="flex-1 overflow-y-auto px-2.5 [&::-webkit-scrollbar]:w-0">
          <div className="px-2 pt-2 pb-1 text-[11.5px] font-medium text-text-faint">{tr("Browse skills")}</div>
          ...
          >
            ...
            <span className="flex-1 text-left">{tr("Built-in skills")}</span>
            ...
            return (
              <div key={s.id}>
                ...
                >
                  ...
                  {!s.enabled && <span className="text-[10px] text-text-faint uppercase">{tr("off")}</span>}
        ...
        {detail ? (
          <SettingsContent wide>
            <SettingsHeader
              ...
              actions={<>
                {editing ? <>
                  <Button variant="ghost" size="sm" disabled={saving} onClick={() => { setEditing(false); setPreview(false); setError('') }}>{tr("Cancelar")}</Button>
                  <Button variant="primary" size="sm" disabled={saving || !dirty || !draft.trim()} onClick={() => void save()}>{saving ? tr("Guardando…") : tr("Guardar")}</Button>
                </> : <Button variant="secondary" size="sm" onClick={edit} disabled={detail.source === undefined}><IconPencil /> {tr("Editar")}</Button>}
                <Toggle on={detail.enabled} disabled={saving} onChange={(v) => toggle(detail.id, v)} />
                <div ref={menu} className="relative">
                  <Button ref={menuButton} variant="ghost" size="icon" title={tr("Más opciones")} aria-label={tr("Más opciones")} aria-haspopup="menu" aria-expanded={menuOpen} disabled={saving} onClick={() => setMenuOpen((open) => !open)}><IconDots /></Button>
                  {menuOpen && (
                    <div role="menu" aria-label={tr("Opciones de la skill")} className="absolute right-0 top-full mt-2 z-20 w-52 p-1.5 rounded-2xl [corner-shape:superellipse(1.5)] border border-white/10 bg-[#323239] shadow-xl"
                      onKeyDown={(event) => {
                      ...
                      <Button role="menuitem" disabled={editing || detail.source === undefined} onClick={edit} className={actionRow}><IconPencil /> {tr("Editar SKILL.md")}</Button>
                      <Button role="menuitem" onClick={() => void openFolder()} className={actionRow}><IconFolder /> {tr("Abrir carpeta")}</Button>
                    </div>
                  )}
                </div>
              </>}
            ...
            <div className="flex gap-16 mt-5">
              <div>
                <div className="text-[12px] text-text-faint mb-1">{tr("Created by")}</div>
                <div className="text-[13.5px] text-text-dim"><Author name={detail.author} /></div>
              </div>
              <div>
                <div className="text-[12px] text-text-faint mb-1">{tr("Last updated at")}</div>
                <div className="text-[13.5px] text-text-dim">{detail.updated || '—'}</div>
                {detail.customized && <div className="mt-1 text-[12px] text-text-faint">{tr("Personalizada")}</div>}
              </div>
            </div>

            {/* Descripción */}
            <div className="mt-5">
              <div className="text-[12px] text-text-faint mb-1.5">{tr("Description")}</div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-4">
                {detail.keywords.map((k) => (
                  <span key={k} className="px-2.5 py-1 rounded-md bg-white/[0.04] border border-white/[0.06] text-[12px] text-text">
                    <span className="text-text-faint">{tr("Keyword")}</span> {k}
                  </span>
                ))}
              </div>
            )}
            ...
            {editing ? (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <Button variant={preview ? 'ghost' : 'secondary'} size="sm" aria-pressed={!preview} onClick={() => setPreview(false)}>{tr("Código")}</Button>
                  <Button variant={preview ? 'secondary' : 'ghost'} size="sm" aria-pressed={preview} onClick={() => setPreview(true)}>{tr("Vista previa")}</Button>
                </div>
                {preview ? (
                  ...
                  <textarea autoFocus aria-label={tr("Contenido de SKILL.md")} spellCheck={false} disabled={saving} value={draft} onChange={(event) => setDraft(event.target.value)}
          ...
          <div className="h-full grid place-items-center text-text-faint text-[14px]">{error || (sel ? tr("Cargando…") : tr("Selecciona una skill."))}</div>
        )}
      </div>
    </div>
  )
